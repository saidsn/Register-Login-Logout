...
for (const product of products) {
  tableBody.innerHTML += `<tr>
    <td product-id="${product.id}"><img src="${product.image}"height="70px" class="card-img-top" alt="..."></td>
    <td>${product.name}</td>
    <td>${product.cost}</td>
    <td>${product.count}</td>
...
deleteIcon.forEach((element) => {
  element.addEventListener("click", function () {
    let id = parseInt(this.parentNode.parentNode.firstElementChild.getAttribute("product-id"));
